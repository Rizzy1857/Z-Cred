...
def demo_unified_scoring():
    ...
    try:
        from trust_score_utils import get_unified_trust_scores, clear_trust_score_cache, get_cache_stats
...
def show_impact_summary():
    ...
    improvements = [
        ("Reproducible Environment", "MEDIUM", "+5-8%", "Pinned dependencies, automated setup"),
        ("SHAP Caching", "HIGH", "+8-12%", "Pre-computed explainers, sub-second responses"),
        ("Database Transactions", "MEDIUM", "+4-6%", "WAL mode, retry logic, better concurrency"),
        ("Unified Scoring Tests", "MEDIUM", "+4-6%", "Validated consistency, regression prevention"),
        ("Performance Profiling", "LOW-MEDIUM", "+2-4%", "Identified and eliminated blocking operations")
    ]
    
    total_min = sum(int(imp[2].split('-')[0].replace('+', '').replace('%', '')) for imp in improvements)
