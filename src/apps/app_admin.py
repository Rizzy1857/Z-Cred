...
class ZScoreAdminApp:
    ...
    def show_system_status(self):
        ...
        try:
            ...
            if ml_result and ml_result.get('overall_trust_score', 0) > 0.1:
                st.markdown('<span class="status-active">🤖 ML Active</span>', unsafe_allow_html=True)
            else:
                st.markdown('<span class="status-warning">🤖 ML Limited</span>', unsafe_allow_html=True)
        except:
            st.markdown('<span class="status-error">🤖 ML Error</span>', unsafe_allow_html=True)
        
        # Database status
        try:
            applicants = self.db.get_all_applicants()
            if len(applicants) > 0:
                st.markdown('<span class="status-active">💾 DB Active</span>', unsafe_allow_html=True)
            else:
                st.markdown('<span class="status-warning">💾 DB Empty</span>', unsafe_allow_html=True)
        except:
            st.markdown('<span class="status-error">💾 DB Error</span>', unsafe_allow_html=True)
    
    def show_system_overview(self):
        """System overview dashboard"""
        st.markdown('<h1 class="admin-header">📊 System Overview Dashboard</h1>', unsafe_allow_html=True)
        ...
        with col3:
            avg_trust = np.mean([a.get('overall_trust_score', 0) for a in applicants]) if applicants else 0
            st.metric("Avg Trust Score", f"{avg_trust:.1%}")
        
        with col4:
            recent = len([a for a in applicants if self.is_recent(a.get('created_at', ''))])
            st.metric("New This Week", recent)
        
        # Trust score distribution
        st.markdown('<h2 class="section-header">📈 Trust Score Distribution</h2>', unsafe_allow_html=True)
        ...
        if applicants:
            ...
            with col2:
                ...
                categories = {
                    ...
                    'Medium Risk (40-69%)': len([s for s in trust_scores if 40 <= s < 70]),
        ...
        recent_applicants = sorted(applicants, key=lambda x: x.get('created_at', ''), reverse=True)[:5]
            ...
            df = pd.DataFrame([
                {
                    ...
                    'Created': a.get('created_at', 'N/A')[:10] if a.get('created_at') else 'N/A'
                }
    ...
    def show_user_management(self):
        ...
        st.markdown('<h1 class="admin-header">👥 User Management</h1>', unsafe_allow_html=True)
        ...
        with col3:
            sort_by = st.selectbox(
                "Sort By",
                ["Trust Score", "Name", "Created Date", "Income"]
            )
        ...
        if trust_filter > 0:
            filtered_applicants = [a for a in filtered_applicants 
        ...
        if sort_by == "Trust Score":
            filtered_applicants.sort(key=lambda x: x.get('overall_trust_score', 0), reverse=True)
        elif sort_by == "Name":
            filtered_applicants.sort(key=lambda x: x.get('name', ''))
        elif sort_by == "Created Date":
            filtered_applicants.sort(key=lambda x: x.get('created_at', ''), reverse=True)
        elif sort_by == "Income":
            filtered_applicants.sort(key=lambda x: x.get('monthly_income', 0), reverse=True)
        ...
        if filtered_applicants:
            ...
            for applicant in filtered_applicants:
                ...
                df_data.append({
                    ...
                    'Status': applicant.get('credit_application_status', 'not_applied'),
                    'Created': applicant.get('created_at', 'N/A')[:10] if applicant.get('created_at') else 'N/A'
                })
    ...
    def show_user_details(self, user):
        ...
        with col2:
            ...
            st.write(f"**Status:** {user.get('credit_application_status', 'not_applied')}")
    ...
    def show_ml_analytics(self):
        ...
        st.markdown('<h1 class="analytics-header">� Advanced ML Intelligence Platform</h1>', unsafe_allow_html=True)
    ...
    def show_advanced_model_performance(self):
        ...
        with col1:
            ...
            performance_matrix = np.random.uniform(0.85, 0.98, size=(len(dates), len(hours)))
        ...
        with col2:
            ...
            metrics = ['Accuracy', 'Precision', 'Recall', 'F1-Score', 'Speed', 'Stability']
    ...
    def show_feature_analysis(self):
        ...
        with col1:
            ...
            features = ['Credit History', 'Income Stability', 'Debt-to-Income', 'Employment Length', 
                       'Account Age', 'Payment History', 'Credit Mix', 'Recent Inquiries']
        ...
        with col2:
            ...
            correlation_matrix = np.random.uniform(-0.8, 0.8, size=(len(features), len(features)))
        ...
        fig_drift = make_subplots(
            rows=1, cols=3,
            subplot_titles=list(feature_values.keys())
        )
    ...
    def show_drift_detection(self):
        ...
        with col1:
            drift_score = 0.02
            status = "🟢 STABLE" if drift_score < 0.05 else "🟡 MONITOR" if drift_score < 0.1 else "🔴 ALERT"
            st.metric("Population Drift", f"{drift_score:.3f}", delta=f"{status}")
        
        with col2:
            concept_drift = 0.008
            status = "🟢 STABLE" if concept_drift < 0.02 else "🟡 MONITOR" if concept_drift < 0.05 else "🔴 ALERT"
            st.metric("Concept Drift", f"{concept_drift:.3f}", delta=f"{status}")
        
        with col3:
            data_quality = 0.97
            status = "🟢 EXCELLENT" if data_quality > 0.95 else "🟡 GOOD" if data_quality > 0.90 else "� POOR"
            st.metric("Data Quality", f"{data_quality:.2%}", delta=f"{status}")
        
        with col4:
            prediction_stability = 0.94
            status = "🟢 STABLE" if prediction_stability > 0.90 else "🟡 MONITOR" if prediction_stability > 0.85 else "🔴 UNSTABLE"
            st.metric("Prediction Stability", f"{prediction_stability:.2%}", delta=f"{status}")
        ...
        with col1:
            ...
            # Add threshold lines
            fig_psi.add_hline(y=0.05, line_dash="dash", line_color="orange", 
                             annotation_text="Monitor Threshold")
            fig_psi.add_hline(y=0.1, line_dash="dash", line_color="red", 
    ...
    def show_ab_testing_results(self):
        ...
        with col1:
            ...
            for i, variant in enumerate(test_variants):
                values = np.random.uniform(0.85 + i*0.02, 0.92 + i*0.02, len(metrics))
        ...
        with col2:
            ...
            fig_significance.add_trace(go.Scatter(
                x=days,
                y=p_values,
                mode='lines+markers',
                name='P-value',
            ...
            fig_significance.add_hline(y=0.05, line_dash="dash", line_color="red", 
    ...
    def show_alerts_monitoring(self):
        ...
        alert_data = {
            'Timestamp': ['2024-08-29 13:42:15', '2024-08-29 12:30:22', '2024-08-29 11:15:45'],
        ...
        with col1:
            ...
            fig_resources = go.Figure(go.Bar(
                x=metrics,
                y=utilization,
                marker_color=['#10b981' if u < 70 else '#f59e0b' if u < 90 else '#ef4444' for u in utilization],
        ...
        with col2:
            ...
            fig_response.add_hline(y=20, line_dash="dash", line_color="orange", 
    ...
    def show_predictive_insights(self):
        ...
        with col1:
            # Volume forecasting
            historical_dates = pd.date_range(start='2024-07-01', end='2024-08-29', freq='D')
            forecast_dates = pd.date_range(start='2024-08-30', end='2024-09-15', freq='D')
        ...
        with col2:
            ...
            fig_perf_forecast.add_trace(go.Scatter(
                x=weeks,
                y=accuracy_forecast,
                mode='lines+markers',
    ...
    def show_performance_metrics(self):
        ...
        st.markdown('<h1 class="analytics-header">📈 Business Intelligence Platform</h1>', unsafe_allow_html=True)
    ...
    def show_business_metrics(self):
        ...
        with col2:
            ...
            channels = ['Organic Search', 'Social Media', 'Paid Ads', 'Referrals', 'Email']
        ...
        for i, cohort in enumerate(cohorts):
            ...
            for month in range(1, len(months)):
                if month <= i + 1:  # Only show data for months that have passed
                    ...
                    retention_rates.append(max(20.0, base_retention + np.random.uniform(-5, 5)))
                else:
        ...
        fig_cohort = go.Figure(data=go.Heatmap(
            ...
            showscale=True,
            hoverongaps=False,
            text=[[f'{val:.1f}%' if val > 0 else '' for val in row] for row in retention_data],
            texttemplate='%{text}',
            textfont={"size": 12}
        ))
    ...
    def show_user_analytics(self):
        ...
        with col1:
            ...
            funnel_stages = ['Visits', 'Sign-ups', 'Profile Complete', 'First Prediction', 'Active Users']
            ...
            fig_funnel = go.Figure(go.Funnel(
                ...
                marker={"color": ["#3b82f6", "#06b6d4", "#8b5cf6", "#10b981", "#f59e0b"]}
            ))
    ...
    def show_revenue_intelligence(self):
        ...
        with col1:
            ...
            products = ['Basic Plan', 'Premium Plan', 'Enterprise', 'API Credits', 'Consulting']
            revenue_by_product = [45000, 67000, 89000, 23000, 34000]
            
            fig_product_revenue = go.Figure(data=[go.Pie(
                labels=products,
        ...
        with col2:
            ...
            growth_rates = [(mrr_values[i] - mrr_values[i-1]) / mrr_values[i-1] * 100 
            ...
            fig_mrr.add_trace(
                go.Scatter(x=months, y=growth_rates, name="Growth Rate (%)", 
    ...
    def show_conversion_funnel(self):
        ...
        with col1:
            ...
            colors = ['#ef4444' if i > 0 and values[i]/values[i-1] < 0.8 
                     else '#f59e0b' if i > 0 and values[i]/values[i-1] < 0.9
                     else '#10b981' for i in range(len(values))]
            
            fig_detailed_funnel = go.Figure(go.Funnel(
                y=steps,
                x=values,
        ...
        with col2:
            ...
            sources = ['Organic Search', 'Paid Search', 'Social Media', 'Direct', 'Referral', 'Email']
        ...
        hours = list(range(24))
        hourly_conversions = [4.2, 3.1, 2.8, 2.5, 2.9, 4.1, 6.7, 8.9, 10.2, 11.8, 
                             12.4, 13.1, 12.9, 12.2, 11.8, 11.4, 10.9, 9.7, 8.8, 7.6, 6.9, 6.1, 5.4, 4.8]
        ...
        fig_hourly.add_vrect(x0=9, x1=17, fillcolor="rgba(59, 130, 246, 0.2)", 
    ...
    def show_performance_optimization(self):
        ...
        with col1:
            # API response times
            endpoints = ['Authentication', 'Predictions', 'User Profile', 'Analytics', 'Reports']
            ...
            fig_api.add_trace(go.Bar(
                ...
                x=endpoints,
                y=response_times,
                marker_color=['#10b981' if rt < sla else '#f59e0b' if rt < sla * 1.2 else '#ef4444' 
        ...
        with col2:
            ...
            fig_db.add_trace(
                go.Bar(x=query_types, y=avg_times, name="Avg Time (ms)", marker_color='#3b82f6'),
                secondary_y=False,
            )
            
            fig_db.add_trace(
                go.Scatter(x=query_types, y=query_counts, name="Query Count", 
                          line=dict(color='#f59e0b', width=3), mode='lines+markers'),
                secondary_y=True,
            )
        ...
        with col3:
            ...
            fig_cache = go.Figure(go.Bar(
                x=cache_types,
                y=hit_rates,
                marker_color=['#10b981' if hr > 90 else '#f59e0b' if hr > 80 else '#ef4444' 
                             for hr in hit_rates],
        ...
        cpu_usage = [45 + 10 * np.sin(0.2 * d) + d * 0.5 + np.random.uniform(-2, 2) for d in days]
        memory_usage = [62 + 8 * np.sin(0.15 * d) + d * 0.3 + np.random.uniform(-1.5, 1.5) for d in days]
        ...
        fig_resources.add_hline(y=90, line_dash="dash", line_color="red", 
    ...
    def show_risk_assessment(self):
        ...
        st.markdown('<h1 class="admin-header">🔍 Risk Assessment Dashboard</h1>', unsafe_allow_html=True)
        
        applicants = self.db.get_all_applicants()
        
        # Risk distribution
        st.markdown('<h2 class="section-header">⚠️ Risk Distribution</h2>', unsafe_allow_html=True)
    ...
    def show_shap_dashboard(self):
        ...
        st.markdown('<h1 class="admin-header">🎯 AI Explanations Dashboard</h1>', unsafe_allow_html=True)
        ...
        # Applicant selection
        applicant_options = {f"{a['name']} (ID: {a['id']})": a['id'] for a in applicants}
        selected_applicant_key = st.selectbox(
            "Select Applicant for AI Explanation",
            list(applicant_options.keys())
        )
        
        selected_applicant_id = applicant_options[selected_applicant_key]
        selected_applicant = next(a for a in applicants if a['id'] == selected_applicant_id)
        ...
        try:
            ...
            if trust_score >= 70:
                st.success("✅ **High Trust Score** - Strong creditworthiness indicators")
            elif trust_score >= 40:
                st.warning("⚠️ **Medium Trust Score** - Some improvement areas identified")
            else:
                st.error("❌ **Low Trust Score** - Significant risk factors present")
    
    def show_system_settings(self):
        """System settings and configuration"""
        st.markdown('<h1 class="admin-header">⚙️ System Settings</h1>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            credit_threshold = st.slider("Credit Eligibility Threshold (%)", 0, 100, 70, 5)
            high_risk_threshold = st.slider("High Risk Threshold (%)", 0, 100, 40, 5)
            
            st.markdown("#### System Parameters")
            max_users = st.number_input("Max Concurrent Users", value=1000, min_value=100)
    ...
    def show_compliance_monitor(self):
        ...
        st.markdown('<h1 class="admin-header">📋 Compliance Monitor</h1>', unsafe_allow_html=True)
        
        # DPDPA compliance status
        st.markdown('<h2 class="section-header">🛡️ DPDPA 2023 Compliance</h2>', unsafe_allow_html=True)
        ...
        for item, status, description in compliance_items:
            ...
            with col2:
                if status == "Active":
                    ...
                    st.markdown('<span class="status-warning">⚠️ Issues</span>', unsafe_allow_html=True)
            
            with col3:
                st.write(description)
        
        # RBI compliance
        st.markdown('<h2 class="section-header">🏛️ RBI Guidelines Compliance</h2>', unsafe_allow_html=True)
        ...
        for item, status, description in rbi_items:
            ...
            with col2:
                st.markdown('<span class="status-active">✅ Compliant</span>', unsafe_allow_html=True)
            
            with col3:
                st.write(description)
        
        # Compliance metrics
        st.markdown('<h2 class="section-header">📊 Compliance Metrics</h2>', unsafe_allow_html=True)
    ...
    def show_data_management(self):
        ...
        st.markdown('<h1 class="admin-header">🔄 Data Management</h1>', unsafe_allow_html=True)
        ...
        with col3:
            scored_users = len([a for a in applicants if a.get('overall_trust_score', 0) > 0])
            st.metric("Scored Users", scored_users)
        
        with col4:
            recent_updates = len([a for a in applicants if self.is_recent(a.get('updated_at', ''))])
            st.metric("Recent Updates", recent_updates)
        
        # Data operations
        st.markdown('<h2 class="section-header">⚙️ Data Operations</h2>', unsafe_allow_html=True)
        ...
        st.markdown('<h2 class="section-header">🔬 Sample Data Management</h2>', unsafe_allow_html=True)
    ...
    def validate_models(self):
        ...
        with st.spinner("Validating models..."):
            time.sleep(2)
            st.success("Model validation completed! All models performing within expected ranges.")
    ...
    def show_user_edit_form(self, user):
        ...
        with st.form(f"edit_user_{user['id']}"):
            ...
            monthly_income = st.number_input("Monthly Income", value=user.get('monthly_income', 0))
