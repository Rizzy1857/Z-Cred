...
from error_handling import ModelError, FeatureExtractionError, ValidationError
...
def test_enhanced_model():
    ...
    try:
        ...
        if 'error' not in explanation:
            ...
            print(f"   - Explanation quality: {explanation.get('explanation_quality', 'unknown')}")
            print(f"   - Top positive contributors: {len(explanation.get('top_contributors', {}).get('positive', []))}")
            print(f"   - Top negative contributors: {len(explanation.get('top_contributors', {}).get('negative', []))}")
    ...
    try:
        ...
        print(f"   - Overall Trust Score: {trust_result.get('overall_trust_score', 'N/A'):.3f}")
        print(f"   - Behavioral Score: {trust_result.get('behavioral_score', 'N/A'):.3f}")
    ...
    try:
        ...
        print(f"✅ Loaded model prediction successful: {test_prediction['risk_category']}")
    except Exception as e:
...
def test_error_handling_module():
    ...
    from error_handling import (
        safe_json_parse, safe_numeric_conversion, 
    ...
    for func, value, expected in validation_tests:
        try:
            ...
            if result == expected:
                ...
                print(f"⚠️  Validation {func.__name__}: {value} -> {result} (expected {expected})")
        except Exception as e:
