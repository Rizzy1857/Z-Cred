...
def test_model_integration():
    ...
    try:
        from model_integration import model_integrator, get_enhanced_trust_assessment
        ...
        for i, test_case in enumerate(test_cases, 1):
            ...
            try:
                ...
                print(f"   Overall Trust: {trust_result.get('trust_percentage', 0):.1f}%")
                print(f"   Behavioral: {trust_result.get('behavioral_score', 0)*100:.1f}%")
            ...
            try:
                combined_result = model_integrator.get_combined_assessment(test_case['data'])
                if 'error' not in combined_result:
                    ...
                    print(f"   Trust Score: {trust_data.get('trust_percentage', 0):.1f}%")
                    print(f"   Risk Category: {risk_data.get('risk_category', 'Unknown')}")
                    print(f"   Risk Probability: {risk_data.get('risk_probability', 0)*100:.1f}%")
                    ...
                    print(f"❌ Combined Assessment Error: {combined_result.get('error')}")
...
def test_model_pipeline_performance():
    ...
    try:
        ...
        start_time = time.time()
        combined_result = model_integrator.get_combined_assessment(test_data)
